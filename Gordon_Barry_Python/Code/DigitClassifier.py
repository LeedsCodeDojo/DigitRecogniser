...
def Main():
    ...
    print(len(dataNumbers))
    dataRecords = [DigitRecord(data) for data in dataNumbers]
    print(len(dataRecords))
    ...
    #print(dataRecords[0].label)
    #Visualiser.Draw("title", dataRecords[0].pixels)
    ...
    trainingRecords, validationRecords = dataRecords[:1600], dataRecords[1600:]
    ...
    print(calculateDistance(trainingRecords[0].pixels, validationRecords[0].pixels))
    ...
    for testRecord in validationRecords:
        classifiedDigit = classify(trainingRecords, testRecord.pixels)
        tested += 1
        if classifiedDigit == testRecord.label:
            matched += 1
        else:
            print(tested, classifiedDigit, testRecord.label)

    print('{}/{} = {}%'.format(matched, tested, matched/tested*100))
...
def calculateDistance (testDigit, knownDigit):
    ...
    return sum([(x-y)*(x-y) for x,y in zip(testDigit, knownDigit)])


def classify (trainingData, unknownPixels, k=3):
    ...
    classifiedData = [(digit, calculateDistance(digit.pixels, unknownPixels)) for digit in trainingData]

    classifiedData = sorted(classifiedData, key=lambda x: x[1])
    minDigit, minDistance = classifiedData[0]
    
    minDigits = [digit for digit, distance in classifiedData[:k] if distance < minDistance*1.5]

    best = {}
    for digit in minDigits:
        best[digit.label] = best.setdefault(digit.label, 0) + 1

    best = list(best.items())
    bestDigit = sorted(best, key=lambda x: x[1], reverse=True)

    if len(bestDigit) == len(minDigits):
        return minDigit.label

    return bestDigit[0][0]

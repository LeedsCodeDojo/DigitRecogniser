...
let records = dataNumbers |> Array.map(fun d -> {Label = d.[0]; Pixels = d.[1..]})
...
//Visualiser.draw (records.[0].Label.ToString()) records.[0].Pixels
...
let trainingData = records.[..1599]
let validationData = records.[1600..]
...
let distance (digit1:int[]) (digit2:int[]) = 
    let diffs = Array.map2(fun d1 d2 -> d1 - d2) digit1 digit2 |> Array.map(fun d -> d * d)
    Array.sum(diffs)
...
let classify (digit:int[]) (nearestx:int) = 
    let nearest = (trainingData 
                        |> Array.map(fun d -> d, (distance d.Pixels digit)) 
                        |> Array.sortBy(fun d -> snd d)).[..(nearestx - 1)]
                        |> Array.map(fun d -> fst d)

    let max = nearest 
                |> Array.toSeq 
                |> Seq.groupBy(fun d -> d.Label) 
                |> Seq.maxBy(fun d -> fst d)
                              
    ((snd max) |> Seq.head).Label
...
//validationData |> Array.iter(fun d -> printfn "digit %d predicted %d" d.Label (classify d.Pixels))

let nearestTest = 1

validationData |> Array.iteri(fun i d -> let predicted = classify d.Pixels nearestTest 
                                         printfn "digit %d predicted %d record %d" d.Label predicted i)
...
validationData |> Array.averageBy(fun d -> if d.Label = classify d.Pixels nearestTest then 1. else 0.)
